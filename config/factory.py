...
###Aldeamo INI - importing libraries for long messages support 
###Aldeamo - install pip install redis
import redis
import binascii
...
from jasmin.vendor.smpp.pdu.gsm_types import IEConcatenatedSM
###Aldeamo END - importing libraries for long messages support
...
class SMPPServerFactory(_SMPPServerFactory):
    ...
    ###Aldeamo - Var: decode pdu headers - long sms support
    ...
    def __init__(self, config, auth_portal, RouterPB=None, SMPPClientManagerPB=None,
        ...
        ###Aldeamo - Var: Redis client, cache for long messages struct - long sms support  
        self.redisClientCache = redis.StrictRedis(host='localhost', port=6379, db=0)
   ...
   ###Aldeamo - Func: detect if contains UDH - long sms support
    ...
    def findConcatenatedSMInfoElement(self, udh):
        ...
        if len(iElems) == 1:
            return iElems[0]
        return None
    ###Aldeamo - Func: get heder - long sms support
    def getHeaderRouteInfo(self, shortMessage):
        headerLen = struct.unpack('!B', shortMessage[0])[0]
        udhBytes = shortMessage[:headerLen+1]                
        udh = self.decodeUDH(udhBytes)                
        hetr = self.findConcatenatedSMInfoElement(udh)
        return hetr[1]

    ###Aldeamo - Func: handle the long sms validation and storage - long sms support
    def longSmsHandler(self, routable):
        sendMessage = True  
        ##Check if is a long message 
        if self.containsUDH(routable.pdu):
            sendMessage = False                
            self.log.debug("[Aldeamo] long message detected for gsm %s ",routable.pdu.params['destination_addr'])               
            headRoute = self.getHeaderRouteInfo(routable.pdu.params['short_message'])
            if isinstance(headRoute, IEConcatenatedSM):
                keyCache = str(routable.pdu.params['destination_addr']) + "-" + str(headRoute.referenceNum)
                self.log.debug("[Aldeamo] long message keyCache: %s ",keyCache)        
                
                ##Get previous sms structure
                messageInCache = self.redisClientCache.get(keyCache)
                
                if messageInCache is None:
                    self.log.debug("[Aldeamo] message not found in cache for keyCache: %s ", keyCache )
                    messageInCache = dict()
                    messageInCache['referenceNum'] = str(headRoute.referenceNum)
                    messageInCache = pickle.dumps(messageInCache)
                    self.redisClientCache.set(keyCache, messageInCache, ex=60)
                    self.log.debug("[Aldeamo] message stored in cache for keyCache: %s ", keyCache )
                
                messageInCache = pickle.loads(messageInCache)
                self.log.debug("[Aldeamo] long message struct loaded  %s ", messageInCache )
                #Add sequence to message structure
                messageInCache[headRoute.sequenceNum] = routable.pdu.params['short_message']

                #Validate if the message struct contains all message parts
                if len(messageInCache) == (headRoute.maximumNum + 1):
                    messageToSend = ""
                    for indexMessage in range(1, headRoute.maximumNum+1):
                        # Concatenate all parts in une string  ([6:] ->  necesary due we need remove the headers in the parts)                          
                        messageToSend += str(messageInCache[indexMessage])[6:]                            

                    self.redisClientCache.delete(keyCache)
                    routable.pdu.params['short_message'] = messageToSend
                    self.log.debug("[Aldeamo] long message to send %s, for keyCache: %s, was deleted from redis cache", messageToSend, keyCache )
                    sendMessage = True
                else:
                    messageInCache = pickle.dumps(messageInCache)
                    self.redisClientCache.set(keyCache, messageInCache, ex=60)
                    self.log.debug("[Aldeamo] long message doesnt contains all parts, storing part in struct for keyCache %s ", keyCache )
        return sendMessage , routable
    ...
    def submit_sm_post_interception(self, *args, **kw):
        ...
        ###Aldeamo - Var: send message - on long sms detected do not send - long sms support 
        sendMessage = True
        try:
            ...
            ###Aldeamo INI: long sms support
            sendMessage , routable = self.longSmsHandler(routable)

            ##Send only when is a short message or the long message contains all parts
            if sendMessage:

                self.log.debug("[Aldeamo] Sending message PDU: %s ", routable.pdu )
                ...
                self.log.debug("[Aldeamo] Do Not send message (long message, waiting all parts, PDU: %s ", routable.pdu )
                
                #routedConnector.cid is setted to "" to avoid adding to RabbitMQ
                c = self.SMPPClientManagerPB.perspective_submit_sm(
                cid="",
                SubmitSmPDU=routable.pdu,
                submit_sm_bill=bill,
            ...
            if message_id is not None and sendMessage:
