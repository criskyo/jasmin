...
class SMPPClientManagerPB(pb.Avatar):
    ...
    def perspective_connector_add(self, ClientConfig):
        ...
        ###Aldeamo INI - Adding durable declaration for rabbitMQ queues
        yield self.amqpBroker.named_queue_declare(queue=submit_sm_queue,durable=True)
        ###Aldeamo END - Adding durable declaration for rabbitMQ queues
    ...
    def perspective_submit_sm(self, cid, SubmitSmPDU, submit_sm_bill, priority=1, validity_period=None,
        ...
        ###Aldeamo INI - Set default response on empty connector (long sms support)
        if cid == "" :
            self.log.debug("Empty Connector Do nothing, PDU %s", SubmitSmPDU)
            defer.returnValue(False)
        ###Aldeamo END - Set default response on empty connector (long sms support)
